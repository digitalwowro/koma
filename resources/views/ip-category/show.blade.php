...
@section('content')
    <section class="content-header">
        <h1>IP Category<small>View {{ str_singular($category->title) }}</small></h1>

        <ol class="breadcrumb">
            <li><a href="{{ route('home') }}">Home</a></li>
            <li><span>Devices</span></li>
            <li><a href="{{ route('device.index', $category->id) }}"><span>{{ $category->title }}</span></a></li>
            <li class="active"><span>View {{ str_singular($category->title) }}</span></li>
        </ol>
    </section>
    ...
    <section class="content">
        <div class="box box-info">
            <div class="box-header with-border">
                <h3 class="box-title">{{ str_singular($category->title) }} Details</h3>
            </div>
